...
docker rm -f test_worktree_docker-main >/dev/null 2>&1 || true
...
echo "Running: worktree_docker --rebuild blooop/test_worktree_docker date to force a rebuild"
start_time=$(date +%s)
worktree_docker --rebuild blooop/test_worktree_docker date
end_time=$(date +%s)
...
echo "Running: worktree_docker blooop/test_worktree_docker date should finish fast (reusing existing container)"
start_time=$(date +%s)
worktree_docker blooop/test_worktree_docker date
end_time=$(date +%s)
...
echo "Running: worktree_docker --nocache blooop/test_worktree_docker date"
start_time=$(date +%s)
worktree_docker --nocache blooop/test_worktree_docker date
end_time=$(date +%s)
