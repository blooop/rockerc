...
echo "Running: worktree_docker --rebuild blooop/test_wtd date to force a rebuild"
start_time=$(date +%s)
worktree_docker --rebuild blooop/test_wtd date
end_time=$(date +%s)
...
echo "Running: worktree_docker blooop/test_wtd date should finish fast (reusing existing container)"
start_time=$(date +%s)
worktree_docker blooop/test_wtd date
end_time=$(date +%s)
...
echo "Running: worktree_docker --nocache blooop/test_wtd date"
start_time=$(date +%s)
worktree_docker --nocache blooop/test_wtd date
end_time=$(date +%s)
