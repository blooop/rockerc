...
# This test verifies that worktree_docker can create worktrees for new branches that don't exist yet
...
cleanup() {
    ...
    # Use worktree_docker prune to clean up properly
    worktree_docker --prune 2>/dev/null || true
    # Fallback cleanup in case prune fails
    docker container prune -f --filter "label=worktree_docker" 2>/dev/null || true
    rm -rf ~/.worktree_docker 2>/dev/null || true
}
...
echo "Testing worktree_docker with new branch 'new_branch' that doesn't exist yet..."

# Run worktree_docker with a new branch and execute git status
worktree_docker blooop/test_worktree_docker@new_branch git status
...
OUTPUT=$(worktree_docker blooop/test_worktree_docker@new_branch git status 2>&1)
...
if docker ps --format "table {{.Names}}" | grep "test_worktree_docker-new-branch"; then
...
worktree_docker --prune blooop/test_worktree_docker@new_branch
...
if [ -d ~/.worktree_docker/workspaces/blooop/test_worktree_docker/worktree-new_branch ]; then
...
worktree_docker blooop/test_worktree_docker@new_branch git status > /dev/null
...
# Verify all worktree_docker containers are gone
...
# Verify .worktree_docker directory is gone
...
FINAL_OUTPUT=$(worktree_docker blooop/test_worktree_docker@new_branch git status 2>&1)
