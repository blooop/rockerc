...
echo "Running: renv blooop/test_wtd to confirm git status is clean (no staged deletions or untracked files)"

# First clean up any untracked files from previous tests
echo "Cleaning up any leftover files from previous tests..."
renv blooop/test_wtd "git reset --hard HEAD; git clean -fd" 2>/dev/null || true

# Run a command that outputs a marker before and after git status to isolate the git output
full_output=$(renv blooop/test_wtd "echo '=== GIT_STATUS_START ==='; git status --porcelain; echo '=== GIT_STATUS_END ==='" 2>/dev/null)
...
if [ -n "$filtered_output" ]; then
    ...
    renv blooop/test_wtd git status 2>/dev/null
    exit 1
fi
