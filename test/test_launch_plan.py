...
def test_prepare_launch_plan_reuse_existing_container():
    ...
    with patch("rockerc.core.container_exists", return_value=True):
        plan = prepare_launch_plan(_base_args(), "", name, vscode=False, force=False, path=pathlib.Path("."))
    assert plan.container_name == name
    assert plan.rocker_cmd == []  # reuse means no rocker invocation
    assert plan.created is False


def test_prepare_launch_plan_force_renames_and_recreates():
    name = derive_container_name("example")
    # Simulate exists -> rename -> not exists path
    with patch("rockerc.core.container_exists", side_effect=[True, False]):
        with patch("subprocess.run") as mock_run:
            plan = prepare_launch_plan(_base_args(), "", name, vscode=True, force=True, path=pathlib.Path("."))
    # First run call should be docker rename
    rename_calls = [c for c in mock_run.call_args_list if c.args[0][:2] == ["docker", "rename"]]
    assert rename_calls, "Expected a docker rename call when force=True and container exists"
...
def test_prepare_launch_plan_injects_required_flags():
    ...
    with patch("rockerc.core.container_exists", return_value=False):
        plan = prepare_launch_plan(_base_args(), "--env FOO=bar", name, vscode=False, force=False, path=pathlib.Path("."))
