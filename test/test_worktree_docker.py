...
from rockerc.worktree_docker import (
    RepoSpec,
    Extension,
    RenvConfig,
    ExtensionManager,
...
class TestPathHelpers:
    ...
    def test_get_workspaces_dir(self):
        ...
        with patch("rockerc.worktree_docker.get_cache_dir", return_value=Path("/cache")):
    ...
    def test_get_repo_dir(self):
        ...
        with patch("rockerc.worktree_docker.get_workspaces_dir", return_value=Path("/workspaces")):
    ...
    def test_get_worktree_dir(self):
        ...
        with patch("rockerc.worktree_docker.get_repo_dir", return_value=Path("/repo")):
...
class TestGitOperations:
    ...
    @patch("rockerc.worktree_docker.setup_bare_repo")
    @patch("subprocess.run")
    @patch("pathlib.Path.exists")
...
class TestComposeOperations:
    ...
    def test_destroy_environment(self, mock_run):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("rockerc.worktree_docker.get_build_cache_dir", return_value=build_dir):
                spec = RepoSpec("owner", "repo", "main")
...
class TestCommands:
    ...
    @patch("rockerc.worktree_docker.launch_environment")
    ...
    @patch("rockerc.worktree_docker.list_active_containers")
    ...
    @patch("rockerc.worktree_docker.prune_all")
    @patch("rockerc.worktree_docker.prune_repo_environment")
    ...
    @patch("rockerc.worktree_docker.ExtensionManager")
...
class TestMainFunction:
    ...
    @patch("rockerc.worktree_docker.cmd_launch")
    ...
    @patch("rockerc.worktree_docker.cmd_list")
...
class TestIntegration:
    ...
    def test_launch_environment_full_workflow(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("rockerc.worktree_docker.get_cache_dir", return_value=cache_dir):
                with patch("rockerc.worktree_docker.get_worktree_dir", return_value=worktree_dir):
                    with patch("rockerc.worktree_docker.get_repo_dir", return_value=repo_dir):
                        spec = RepoSpec("owner", "repo", "main")
