...
def build_docker(dockerfile_path: str = ".", buildx: bool = True) -> str:
    ...
    Args:
        ...
        buildx (bool, optional): Enable Docker Buildx via DOCKER_BUILDKIT=1. Defaults to True.
    ...
    build_cmd = ["docker", "build", "-t", tag, str(dockerfile_dir)]

    env = os.environ.copy()
    if buildx:
        env["DOCKER_BUILDKIT"] = "1"

    subprocess.call(build_cmd, env=env)
    return tag
...
def _parse_extra_flags(argv: List[str]) -> Tuple[bool, bool, bool, bool, List[str]]:
    """Parse ad-hoc flags for --vsc, --force, --verbose and --buildx.

    Returns: (vsc, force, verbose, buildx, remaining_args)
    """
    vsc = False
    force = False
    verbose = False
    buildx = True  # Enabled by default
    remaining: List[str] = []
    for a in argv:
        if a == "--vsc":
            vsc = True
        ...
        if a == "--buildx":
            ...
            continue
        remaining.append(a)
    return vsc, force, verbose, buildx, remaining
...
def run_rockerc(path: str = "."):
    ...
    cli_args = sys.argv[1:]
    vsc, force, verbose, buildx, filtered_cli = _parse_extra_flags(cli_args)
    ...
    if "dockerfile" in merged_dict:
        ...
        merged_dict["image"] = build_docker(merged_dict["dockerfile"], buildx=buildx)
