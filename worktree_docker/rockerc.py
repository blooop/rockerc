...
def collect_arguments(path: str = ".") -> dict:
    """Search for worktree_docker.yaml files and return a merged dictionary

    Args:
        path (str, optional): path to reach for files. Defaults to ".".

    Returns:
        dict: A dictionary of merged worktree_docker arguments
    """
    search_path = pathlib.Path(path)
    merged_dict = {}
    for p in search_path.glob("worktree_docker.yaml"):
        print(f"loading {p}")
...
def save_rocker_cmd(split_cmd: str):
    ...
    try:
        ...
        with open("Dockerfile.rocker", "w", encoding="utf-8") as dockerfile:
            dockerfile.write("#This file was autogenerated by worktree_docker\n")  # Add the shebang
        ...
        formatted_script_lines.append("# This file was autogenerated by worktree_docker")
        ...
        logging.error("[worktree_docker] Error: rocker dry-run failed.")
        logging.error(f"[worktree_docker] Command: {' '.join(dry_run)}")
        logging.error(f"[worktree_docker] Exit code: {e.returncode}")
        logging.error(f"[worktree_docker] Output:\n{e.stdout}")
        logging.error(f"[worktree_docker] Error output:\n{e.stderr}")
        logging.error(
            "[worktree_docker] This likely means rocker or one of its extensions failed to generate a Dockerfile. Please check your worktree_docker.yaml and rocker installation."
        )
        sys.exit(e.returncode)
    except ValueError as e:
        logging.error(f"[worktree_docker] Error processing the output from rocker dry-run: {e}")
        logging.error(
            "[worktree_docker] The output format may have changed or rocker failed to generate the expected output."
        )
        sys.exit(1)


def run_worktree_docker(path: str = "."):
    """run worktree_docker by searching for rocker.yaml in the specified directory and passing those arguments to rocker

    Args:
        path (str, optional): Search path for worktree_docker.yaml files. Defaults to ".".
    """
    ...
    if not merged_dict:
        logging.error(
            "No worktree_docker.yaml found in the specified directory. Please create a worktree_docker.yaml file with rocker arguments. See 'rocker -h' for help."
        )
        sys.exit(1)

    if "args" not in merged_dict:
        logging.error(
            "No 'args' key found in worktree_docker.yaml. Please add an 'args' list with rocker arguments. See 'rocker -h' for help."
        )
        sys.exit(1)
    ...
    if len(cmd_args) > 0:
        ...
        logging.error(
            "no arguments found in worktree_docker.yaml. Please add rocker arguments as described in rocker -h:"
        )
